...
import { ArrowRight, CheckCircle2, Phone, Sparkles, ShieldCheck, Wrench, HelpCircle } from "lucide-react";
      ...
      <section className="relative overflow-hidden bg-primary text-primary-foreground">
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <Link to="/contact" className="inline-flex items-center gap-2 rounded-md bg-accent px-5 py-3 text-sm font-semibold text-accent-foreground hover:bg-accent/90">
              Get a free quote <ArrowRight className="h-4 w-4" />
            </Link>
            <a href={SITE.phoneHref} className="inline-flex items-center gap-2 rounded-md border border-primary-foreground/30 bg-primary-foreground/5 px-5 py-3 text-sm font-semibold text-primary-foreground hover:bg-primary-foreground/10">
              <Phone className="h-4 w-4" /> {SITE.phone}
            </a>
          </div>
          <div className="mt-6 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-primary-foreground/70">
            <span className="inline-flex items-center gap-1.5"><ShieldCheck className="h-4 w-4 text-accent" /> Gas Safe {SITE.gasSafe}</span>
            <span className="inline-flex items-center gap-1.5"><ShieldCheck className="h-4 w-4 text-accent" /> OFTEC {SITE.oftec}</span>
              ...
              <Reveal delay={60}>
                <div className="prose prose-sm max-w-none">
                  ...
                  <div className="mt-3 space-y-4 text-[15px] leading-relaxed text-muted-foreground">
                    {details.body.map((p: string, i: number) => <p key={i}>{p}</p>)}
                  </div>
                </div>
              </Reveal>
            )}
            ...
            {details?.process && (
              <Reveal delay={100}>
                ...
                <ol className="mt-5 space-y-4">
                  {details.process.map((step: { title: string; description: string }, i: number) => (
              ...
              <Reveal delay={120}>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {details.brands.map((b: string) => (
                    <span key={b} className="rounded-full border border-border bg-secondary px-4 py-1.5 text-sm font-medium text-secondary-foreground">
                      {b}
                    </span>
                  ))}
                </div>
            ...
            {details?.pricing && (
              <Reveal delay={130}>
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  Every job is different — these are typical starting prices. You'll get a fixed written quote before any work begins.
                </p>
                <div className="mt-5 overflow-hidden rounded-xl border border-border">
                  <table className="w-full text-sm">
                    <tbody>
                      {details.pricing.map((p: { label: string; price: string; note?: string }, i: number) => (
                        <tr key={p.label} className={i % 2 === 0 ? "bg-card" : "bg-secondary/40"}>
                          <td className="px-4 py-3 font-medium text-foreground">{p.label}</td>
                          <td className="px-4 py-3 text-right font-semibold text-primary whitespace-nowrap">{p.price}</td>
                          <td className="hidden px-4 py-3 text-right text-xs text-muted-foreground sm:table-cell">{p.note}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          <aside className="lg:col-span-1">
            <div className="sticky top-24 space-y-4">
              <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
                ...
                <Link to="/contact" className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-md bg-accent px-5 py-3 text-sm font-semibold text-accent-foreground hover:bg-accent/90">
                  Book online <ArrowRight className="h-4 w-4" />
                </Link>
                <a href={SITE.phoneHref} className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-md border border-border bg-background px-5 py-3 text-sm font-semibold text-foreground hover:bg-secondary">
                  <Phone className="h-4 w-4" /> {SITE.phone}
                </a>
              ...
              <div className="rounded-2xl border border-border bg-primary p-6 text-primary-foreground shadow-sm">
                <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-accent">
                  <Wrench className="h-4 w-4" /> Emergency?
                </div>
                <p className="mt-2 text-sm text-primary-foreground/85">
                  24/7 call-outs across Hull & East Yorkshire. No heat, no hot water, or a leak you can't stop — ring the office direct.
                </p>
                <a href={SITE.phoneHref} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-md bg-accent px-5 py-3 text-sm font-semibold text-accent-foreground hover:bg-accent/90">
                  <Phone className="h-4 w-4" /> Call {SITE.phone}
                </a>
              </div>

              <div className="rounded-2xl border border-border bg-secondary/50 p-6">
                <h4 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Coverage</h4>
                <p className="mt-2 text-sm text-foreground">
                  Hull, Beverley, Cottingham, Hessle, Anlaby, Willerby, Hedon, Hornsea, Withernsea, Driffield and the surrounding East Yorkshire villages.
                </p>
              </div>
            </div>
          </aside>
            ...
            <div className="mt-8 grid gap-6 md:grid-cols-3">
              {related.map((r) => (
                ...
                >
                  ...
                  <div className="p-5">
                    <h3 className="font-semibold text-foreground group-hover:text-accent">{r.title}</h3>
